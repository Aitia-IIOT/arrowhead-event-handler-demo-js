 ...
 */
 ...
 export async function subscibe(subscriptionRequest){
   ...
   return new Promise((resolve, reject) => {
     ...
     if(subscriptionRequest.matchMetaData === undefined) {
     ...
     networkService.post(eventHandlerAddress + '/subscribe', subscriptionRequest)
       ...
       .catch(error => {
        ...
        if(errorMessage.includes("Subscription violates unique constraint rules")){
          return resolve("System is already subscribed to the event")
        }
